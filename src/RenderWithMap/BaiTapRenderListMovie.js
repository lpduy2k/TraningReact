...
import style from './BaiTapRenderListMovie.module.css'
// import './BaiTapRenderListMovie.css'


export default class BaiTapRenderListMovie extends Component {
    mangPhim = [
        {
          maPhim: 1321,
          tenPhim: "Ted2",
    ...
    renderProductWithMap = () => {
        ...
        const arrJSX = this.mangPhim.map((product,index) => {
            return (
              <div className="col-4" key={index}>
                ...
                >
                  {/* <img
                    src={product.hinhAnh}
                    className={`card-img-top ${style.hinhAnhPhim}`}
                    alt="..."
                  /> */}
                  <img
                    src={product.hinhAnh}
                  ...
                  <div className="card-body">
                    <h5 className="card-title" style={{height:50}}>{product.tenPhim}</h5>
                <p className="card-text" style={{height:100}}>{product.moTa.length > 100 ? <span>{product.moTa.substr(0, 100)}...</span> : <span>{product.moTa}</span>}</p>
                  </div>
                </div>
              </div>
            );
